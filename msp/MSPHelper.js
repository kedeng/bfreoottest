require('../fc/injected_methods')
const { bit_check, bit_set } = require('../fc/bit')
...
const {
  ...
  API_VERSION_1_46,
  API_VERSION_1_47
} = require('../fc/data_storage')
const EscProtocols = require('../utils/EscProtocols')
const { huffmanDecodeBuf } = require('../utils/huffman')
const { defaultHuffmanTree, defaultHuffmanLenIndex } = require("../utils/default_huffman_tree")
        ...
        if (!dataHandler.unsupported) switch (code) {
            ...
            case MSPCodes.MSP2_GET_VTX_DEVICE_STATUS:
                ...
                if (dataLength > 0) {
                    ...
                    console.log(`warn delete FC.VTX_DEVICE_STATUS = vtxDeviceStatusFactory.createVtxDeviceStatus(vtxDeviceStatusData)`)
            ...
            case MSPCodes.MSP_MOTOR_TELEMETRY:
                ...
                // console.log(FC.MOTOR_TELEMETRY_DATA)
                break;
...
MspHelper.prototype.writeConfiguration = function(reboot, callback) {
    const buffer = [];
    setTimeout(function() {
        MSP.send_message(MSPCodes.MSP_EEPROM_WRITE, false, false, async function() {
            gui_log(i18n.getMessage('configurationEepromSaved'));
            console.log('Configuration saved to EEPROM');
            if (reboot) {
                console.log('Configuration send reboot');
                buffer.push8(0)
                await MSP.promise(MSPCodes.MSP_SET_REBOOT, buffer)
                console.log('Configuration reboot')
                MSP.disconnect_cleanup()
                // reinitializeConnection(callback)
                /*
    ...
    }, 500); // 100ms delay before sending MSP_EEPROM_WRITE to ensure that all settings have been received
};
