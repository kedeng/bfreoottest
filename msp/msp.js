...
const { CONFIGURATOR } = require('../fc/data_storage')
const serial = require('../hardware/fcSerial')
...
const MSP = {
    ...
    read(readInfo) {
        ...
        const data = new Uint8Array(readInfo);
    ...
    send_message(code, data, callback_sent, callback_msp, doCallbackOnError) {
        ...
        const connected = serial.portIsOpened;
        ...
        if (!requestExists) {
            obj.timer = setTimeout(() => {
                console.log(`MSP: data request timed-out: ${code} ID: ${serial.connectionId} TIMEOUT: ${this.timeout} QUEUE: ${this.callbacks.length} (${this.callbacks.map((e) => e.code)})`);
        ...
        if (data || !requestExists) {
            ...
            serial.send(bufferOut, (err) => {
                if (!err) {
                    if (callback_sent) {
                        callback_sent();
                    }
                } else {
                  console.log(`serial send error: ${ err.message }`)
                }
            });
        }
